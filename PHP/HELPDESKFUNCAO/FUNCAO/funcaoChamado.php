<?php

function buscarPorId($conn, $id)
{
    $sql = "SELECT 
            c.id,
            c.titulo,
            c.descricao,
            c.categoria as categoria_id,
            c.preco,
            cat.categoria,
            u.nome
            FROM chamados c
            LEFT JOIN categoria cat ON c.categoria = cat.id
            LEFT JOIN usuarios u ON c.idUser = u.id WHERE c.id = ?";

    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt)
        return false;

    mysqli_stmt_bind_param($stmt, 'i', $id);

    mysqli_stmt_execute($stmt);

    $resul = mysqli_stmt_get_result($stmt);

    $chamados = mysqli_fetch_assoc($resul);

    return $chamados;

}

function buscarTodos($conn)
{
    $sql = "SELECT chamados.*, categoria.categoria, usuarios.nome FROM chamados 
    JOIN usuarios ON chamados.idUser = usuarios.id
    JOIN categoria ON chamados.categoria = categoria.id
    ";

    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt)
        return [];

    mysqli_stmt_execute($stmt);

    $resul = mysqli_stmt_get_result($stmt);

    $chamados = [];

    while ($chamado = mysqli_fetch_assoc($resul)) {
        $chamados[] = $chamado;
    }

    return $chamados;
}

function abrirChamado($conn, $titulo, $categoria, $descricao, $userId)
{
    $sql = 'INSERT INTO chamados (titulo, categoria, descricao, idUser) VALUES(?, ?, ?, ?)';

    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt)
        return false;

    mysqli_stmt_bind_param($stmt, 'sisi', $titulo, $categoria, $descricao, $userId);

    $resul = mysqli_stmt_execute($stmt);

    mysqli_stmt_close($stmt);

    return $resul;

}

function atualizaChamado($conn, $titulo, $categoria, $descricao, $preco, $id)
{

    $sql = 'UPDATE chamados SET titulo = ?, categoria = ?, descricao = ?, preco = ? WHERE id = ?';

    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt)
        return false;

    mysqli_stmt_bind_param($stmt, 'sisid', $titulo, $categoria, $descricao, $preco, $id);

    $resul = mysqli_stmt_execute($stmt);

    mysqli_stmt_close($stmt);

    return $resul;
}

function excluirChamado($conn, $id)
{
    $sql = 'DELETE FROM chamados WHERE id = ?';

    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt)
        return false;

    mysqli_stmt_bind_param($stmt, 'i', $id);

    $resul = mysqli_stmt_execute($stmt);

    mysqli_stmt_close($stmt);

    return $resul;
}